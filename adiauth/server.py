...
from adiauth import USER, TOKEN, ADMIN, ADMIN_TOKEN, USER_TOKEN, HASH_PASS,\
    DEFAULT_PORT, DEFAULT_AUTH_DB, HTTPS_DEBUG_MODE
...
def routeApp(app, AUTHDB, TOKENMAN):
    ...
    def do_login():
        ...
        try:
            token = TOKENMAN.newToken(request_data[USER], request_data[HASH_PASS])
        except Unauthorized:
    ...
    def add_user(user):
        ...
        try:
            AUTHDB.newUser(user, request_data[HASH_PASS])
    ...
    def change_user_password(user):
        ...
        if user_token:
            try:
                expected_user = TOKENMAN.ownerOf(user_token)
            except ObjectNotFound:
                pass

        if ((admin_token == TOKENMAN.admin_token) or (user == expected_user)):
            try:
                AUTHDB.changePasswordHash(user, request_data[HASH_PASS])
...
class AuthService:
    ...
    def __init__(self, db_file, admin_token, host='0.0.0.0', port=DEFAULT_PORT):
